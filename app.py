...
def get_song_gap_graph(band_json, band):
    ...
    show_new_songs_perc_graph = figure(y_range=list(dates), title=band, plot_height=700, plot_width=700,
              toolbar_location=None, tooltips = [("Date", "@Date"), ("Not Played Prev Show",'@NewSongs') , ("2 Show Gap", "@NewSongs_2"), ('3 Show Gap','@NewSongs_3')])

    show_new_songs_perc_graph.hbar_stack(show_gaps_perc, y='Date', height = 0.9, source=source_snsg, legend_label=legend_perc, color=[yellow, orange, red])
    show_new_songs_perc_graph.line(y='Date', x='Expanding Average', line_width=3, source=source_snsg, color=dark_red, legend_label='Average % Songs Not Played in Previous Show')
...
def get_top_song_freq_graph(band_json, band):
    setlist_dict = get_setlist_dict(band_json)
    top_songs, song_counts = get_top_songs(setlist_dict)
    dates = setlist_dict.keys()
    ...
    top_songs_percent_graph = figure(y_range=top_songs_w_avg, title=band, frame_width=400, plot_width=700,
...
def get_new_year_songs_graph(band_json, band):
    setlist_dict = get_setlist_dict(band_json)
    ...
    year_new_songs_graph = figure(y_range = years_asc, title=band,
...
def get_iqr_df(placement_df):
    ...
    avg_iqr_placement = statistics.mean(iqr_of_positions)
...
def get_song_placement_graph(band_json, band):
    ...
    top_songs_w_av = ['Average IQR'] + top_songs
    #placement_df = placement_df.sample(frac=0.1, replace=True, random_state=1)
    source_place = ColumnDataSource(placement_df)
    source_iqr = ColumnDataSource(iqr_df)
    top10_placement = figure(y_range=top_songs_w_av, title=band, tooltips=[('Date',"@date"), ('Show Placement',"@hover")], sizing_mode='stretch_both', toolbar_location=None)
    top10_placement.circle(x='placement', y=jitter('song', width=0.4, range=top10_placement.y_range), radius=0.01, fill_alpha=0.6, source=source_place,
             color=mapper)
    top10_placement.hbar(y="Songs", left='Q1', right='Q3', source = source_iqr, height = 0.4, color = yellow, fill_alpha=0.5)
...
def get_metrics_graph(band_dict, sort_by, bands):

    #metrics_df = get_metrics_df(band_dict)
    metrics_df = pd.read_csv('data/metrics_df.csv')
    metrics_df['Total of all Metrics'] = metrics_df['Song Gaps'] + metrics_df['New Songs Per Year'] + metrics_df['Top 10 Song Frequency'] + metrics_df['Top 10 Song Placement']
    ...
    metrics_graph = figure(y_range=bands, title="Comparison of All Metrics",
                           plot_width=1200, plot_height = 700,
                           toolbar_location=None)

    metrics_graph.hbar_stack(mets, y='Band', source=source_met, legend_label=mets, height=0.9,
                             color=colors)
...
band_dict = {'Vampire Weekend':'vampireweekend', 'Arcade Fire':'arcadefire', 'Beck':'beck', 'Pearl Jam':'pearljam',
             ...
             'Neil Young':'neilyoung', 'Bruce Springteen':'springsteen', 'King Gizzard':'kinggizzard', 'Billy Strings':'billy',
             'Bob Dylan':'dylan', 'Jimmy Buffett':'buffett', 'Dream Theater':'dream', 'Animal Collective':'animal',
             'Wilco':'wilco', 'String Cheese Incident':'sci', 'Medeski Martin Wood':'mmw',
...
band_classic = ['Grateful Dead', 'Metallica', 'Iron Maiden', 'U2', 'The Rolling Stones', 'Neil Young',
                'Bruce Springsteen', 'Bob Dylan', 'Jimmy Buffett', 'Santana', 'Frank Zappa', 'Billy Joel', 'Talking Heads',
                'Van Morrison', 'Tom Petty', 'Steely Dan', 'Stevie Wonder', 'Prince', 'Dream Theater', 'Red Hot Chili Peppers']
band_other = ['Vampire Weekend', 'Arcade Fire', 'Beck', 'Radiohead', 'Foo Fighters', 'Muse', 'Tame Impala',
              'LCD Soundsystem', 'The Avett Brothers', 'King Gizzard', 'Animal Collective', 'Wilco', 'My Morning Jacket',
              'Portugal. the Man', 'Jason Isbell', 'Drive-By Truckers', 'The National', 'Bon Iver', 'Clutch', 'Kendrick Lamar', 'Kanye West',
              'Beyonce', 'Jay-Z', 'Pearl Jam']
bands_less250 = ['Billy Strings', 'Disco Biscuits', 'Goose', 'Greensky Bluegrass', 'Jay-Z', 'Kanye West', 'Medeski Martin Wood']
...
def create_song_position_dict():
    ...
    json = json.dumps(song_position_dict)
    f = open("data/song_position_dict.json", "w")
    f.write(json)
    f.close()

    print(song_position_dict)



genre_select = st.sidebar.selectbox(
    'Band Groups',
    ('All Bands', 'Bands Starting Before 1990', 'Bands Starting After 1990','Jam Bands')
)
if include250:
...
graph_selectbox = st.sidebar.selectbox(
    "Choose a Comparison",
    ('Song Gaps', 'New Songs Per Year', 'Top 10 Song Frequency', 'Top 10 Song Placement', 'Compare Bands Across All Measures')
)

if graph_selectbox == 'Compare Bands Across All Measures':
    sort_by = st.sidebar.selectbox("Sort By:",
    ('Total of all Metrics', 'Song Gaps', 'New Songs Per Year', 'Top 10 Song Frequency', 'Top 10 Song Placement'))
...
if graph_selectbox == 'Song Gaps':
    graph1_a, meta = get_song_gap_graph(band_dict[band_selectbox_a] + '.json', band_selectbox_a)
    graph1_b, metb = get_song_gap_graph(band_dict[band_selectbox_b] + '.json', band_selectbox_b)
    graph1_a.legend.visible = False
    two_graphs = row(graph1_a, graph1_b)
elif graph_selectbox == 'New Songs Per Year':
    graph1_a, meta = get_new_year_songs_graph(band_dict[band_selectbox_a] + '.json', band_selectbox_a)
    graph1_b, metb = get_new_year_songs_graph(band_dict[band_selectbox_b] + '.json', band_selectbox_b)
    two_graphs = row(graph1_a, graph1_b)
elif graph_selectbox == 'Top 10 Song Frequency':
    graph1_a, meta = get_top_song_freq_graph(band_dict[band_selectbox_a] + '.json', band_selectbox_a)
    graph1_b, metb = get_top_song_freq_graph(band_dict[band_selectbox_b] + '.json', band_selectbox_b)
    two_graphs = row(graph1_a, graph1_b)
elif graph_selectbox == 'Top 10 Song Placement':
    graph1_a, meta = get_song_placement_graph(band_dict[band_selectbox_a] + '.json', band_selectbox_a)
    graph1_b, metb = get_song_placement_graph(band_dict[band_selectbox_b] + '.json', band_selectbox_b)
    two_graphs = row(graph1_a, graph1_b)
elif graph_selectbox == 'Compare Bands Across All Measures':
    two_graphs = get_metrics_graph(band_dict, sort_by, bands)
